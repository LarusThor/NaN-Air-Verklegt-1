...
class UpcomingVoyageLL:
    ...
    def calculate_flight_time(self, arr_at: Destination, departure_date_time: datetime) -> datetime:  # TODO: change name to calculate arrival time
        ...
        estimated_flight_time_delta = datetime.strptime(
            estimated_flight_time_str, "%H:%M:%S"
        ).time()
    ...
    def valid_pilot(self, aircraft_id: str, pilot: str) -> bool:  # TF-XUP, 34928348392
        ...
        employee_dict = (
            self.logic.data_wrapper.get_all_staff_members()
        )  # {293919999: Jón, Pilot...}
    ...
    def detect_voyage_info_return_flight(self, voyage_flight_number: str, voyage_date: str) -> UpcomingVoyage:
        """Returns information about return flight from departure flight."""
        flight_info = self.logic.upcoming_voyages()
        for key, item in flight_info.items():
            if (
                voyage_flight_number == item.flight_nr
                and voyage_date in item.departure.strftime("%Y-%m-%d %H:%M:%S")
            ):
                id_of_flight = key
    ...
    def get_voyage_by_flight_nr_and_date(self, flight_nr: str, departure_date: str) -> UpcomingVoyage:
        """Finds a voyage from flight number and date, two flights can have the same flight number but not on the same day."""
        voyages = self.logic.data_wrapper.get_upcoming_flights()

        for voyage in voyages.values():
            if (
                voyage.flight_nr == flight_nr
                and voyage.departure.strftime("%Y-%m-%d") == departure_date
            ):
                return voyage
